/*The tags are arrays containing multiple bits of information.*/
const hikes = [
  {
    name: "Bechler Falls",
    stub: "bechler_falls",
...
/*creates items from the elements in html */
let hikeContainer = document.querySelector('#hike-container');
let button = document.querySelector('button');

button.addEventListener('click', search);

/*This is a function to instruct the search bar to look through and return whatever value is put in the search bar.*/
function search(){
    ...
    /*resets the html to nothing so that the display is fresh information.*/
...
/*generates a random number within the total length of the hikes list and prints it to console.*/
let randomNum = Math.floor(Math.random()* hikes.length);
console.log(randomNum);


/* This function uses a .map to go through every single tag.*/
...
/*This is where the html container for the hikes is put. Use this template for the recipes in the assignment*/
...
/*This will generate a hike based on the random number of the chosen hike. */
function renderHike(hike) {
    let html = hikesTemplate(hike);
    hikeContainer.innerHTML += html
}

/*This is going to run right when the screen opens and do so with the random number we generated earlier. */
function init() {
